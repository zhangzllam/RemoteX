...
The repository currently implements **Milestones 0–5**:
...
Clipboard synchronization, file I/O, the control-server API, persistent device
registration, and P2P are intentionally not implemented yet.
...
[Running M5](docs/running-m5.md) for the current development-only relay, agent,
...
M3–M5 video and input envelopes use XChaCha20-Poly1305 authenticated encryption
...
input events. Remote input is disabled unless the Agent user explicitly enables the
